    ...
    for epoch in range(num_epochs):
        ...
        for batch_idx, batch in progress_bar:
            ...
            with torch.no_grad():
                ...
                # CORRECTED: Get probabilities for positive class (safe = class 1)
            ...
            for i, text in enumerate(texts):
                ...
                # CORRECTED: Update action counts (1 = safe, 0 = unsafe)
                if action == 1:
            ...
            if action_counts["safe"] >= max_action_imbalance * total_actions:
                ...
                for i, action in enumerate(old_action):
                    if action.item() == 0:  # CORRECTED: If unsafe prediction (0)
                ...
                for i, action in enumerate(old_action):
                    if action.item() == 1:  # CORRECTED: If safe prediction (1)
                        batch_rewards[i] *= reward_boost_factor
...
def evaluate_model(model, tokenizer, csv_file, text_column, label_column=None, max_seq_length=128, device=None):
    ...
    results = {
        ...
        "prediction_distribution": {
            "unsafe": all_preds.count(0),  # CORRECTED: 0 = unsafe
            "safe": all_preds.count(1),    # CORRECTED: 1 = safe
            "unsafe_percent": all_preds.count(0) / len(all_preds) * 100,
            "safe_percent": all_preds.count(1) / len(all_preds) * 100
        },
...
if __name__ == "__main__":
    ...
    model, tokenizer, metrics = train_with_ppo(
        ...
        text_column="content",
        label_column="is_safe",  # CORRECTED: Label column name might need adjustment based on your dataset
        output_dir="ppo_safety_classifier",
        batch_size=4,
        num_epochs=5,
        learning_rate=1e-5,
    ...
    evaluation_results = evaluate_model(
        ...
        "safety_dataset.csv",
        "content",
        "is_safe"  # CORRECTED: Label column name might need adjustment
    )
